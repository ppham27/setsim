independent <- function(model, y, fit, cov,B, max_B, n.splits,
  ...
  independentMini <- function(idx, BB, Model) {
    ...
    H.lik <- function(y, X, EST) { Model$H.lik(y, X, EST) }
    EST <- Model$input$EST
    ...
    colnames(output) <-
      c("z","level","n","epsilon",paste("ray.EST",1:p),"log_lik")
  ...
  wald.num <- matrix(c(EST-qnorm(0.975)*sqrt(diag(cov)), EST+qnorm(0.975)*sqrt(diag(cov))),nrow=p,ncol=2,dimnames=list(paste("EST",1:p),c("Lower Bound","Upper Bound")))
  wald.sim <- matrix(c(apply(out_wald[,5:(p+4)],2,min), apply(out_wald[,5:(p+4)],2,max)),nrow=p,ncol=2,dimnames=list(paste("EST",1:p),c("Lower Bound","Upper Bound")))
