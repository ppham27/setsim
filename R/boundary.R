  ...
  function (model, y, fit, target, targetvalue, cov, B, max_B,
    ...
    boundaryMini <- function(idx, BB, Model) {
      ...
      out_c <- .Call("boundaryFixed",
                     H, environment(H),
                     V, z, t(z), critvalue,
                     targetvalue, input, EST,
                     soltype)
      output <- matrix(out_c[[1]],byrow=TRUE,ncol=p+4)
      colnames(output) <- c("z","level","n","epsilon",paste("ray.EST",1:p))
    ...
    conv.waldnum <- matrix(c(EST-qnorm(0.975)*sqrt(diag(cov)),EST+qnorm(0.975)*sqrt(diag(cov))),p,2,dimnames=list(paste("EST",1:p),c("Lower Bound","Upper Bound")))
    conv.waldsim <- matrix(c(apply(out_wald[,5:(p+4)],2,min),apply(out_wald[,5:(p+4)],2,max)),p,2,dimnames=list(paste("EST",1:p),c("Lower Bound","Upper Bound")))
    if (target=="level") {
      ...
      for (i in 1:length(targetvalue)) {
        ...
        wald.num[i,2:(2*p+1)] <-
          matrix(c(EST-sqrt(qchisq(targetvalue[i],p))*sqrt(diag(cov)),
                   EST+sqrt(qchisq(targetvalue[i],p))*sqrt(diag(cov))),
                 1,2*p)
      ...
      colnames(wald.num) <- c("1D.Level",paste("Lower.EST",1:p),paste("Upper.EST",1:p))
      rownames(lik.sim) <- targetvalue
      colnames(lik.sim) <- c("1D.Level",paste("Lower.EST",1:p),paste("Upper.EST",1:p))
      list(EST = EST, diagnosis = soltype, boundary.sample = output,
