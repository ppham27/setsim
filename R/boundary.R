...
output <- matrix(numeric(0), 0, p + 4)
output <- matrix(output,nrow=nrow(output),ncol=ncol(output),dimnames=list(rep("",nrow(output)),c("z","level","n","epsilon",paste("ray.MLE",1:p))))
...
if (!is.null(B)){
  ...
  for (i in 1:dim(z)[1]){
    ...
    for (s in 1:length(critvalue)){
      ...
      range <- roots$range
      sol <- 0
      if (roots$n == 0){soltype[s, 2] <- soltype[s, 2] + 1}
      else {
        for (k in 1:roots$n) {
          sol[k] <- bisect(fn=H,a=range[k,1],b=range[k,2],input=input,VZ=VZ,c_value=c_value)
          output <- rbind(output, c(i,targetvalue[s],roots$n,sol[k],as.vector(MLE+sol[k]*VZ[,1])))}
    ...
    out_wald <- rbind(out_wald, c(i, targetvalue[s], roots$n, -sol_w, as.vector(MLE - sol_w * VZ[,1])))
    out_wald <- rbind(out_wald, c(i, targetvalue[s], roots$n, sol_w, as.vector(MLE + sol_w * VZ[,1])))
...
if (target=="level" || target=="customized"){
  ...
  for (i in 1:length(targetvalue)){
    ...
    wald.num[i,2:(2*p+1)] <- matrix(c(MLE-sqrt(qchisq(targetvalue[i],p))*sqrt(diag(cov)),MLE+sqrt(qchisq(targetvalue[i],p))*sqrt(diag(cov))),1,2*p)
  ...
  list(MLE = MLE, diagnosis = soltype, boundary.sample = output, WaldBoundary.sample = out_wald, ray.z = z, numWald.interval = wald.num, simLik.interval = lik.sim, convnumWald = conv.waldnum, convsimuWald = conv.waldsim)}
else if (target=="ratio"){list(MLE = MLE, diagnosis = soltype, boundary.sample = output, boundaryWald.sample = out_wald, ray.z = z, convnumWald = conv.waldnum, convsimuWald = conv.waldsim)}
}